...
public class Dimagotchi {
    ...
    private int energy;
    private int cleanliness; // 청결도 (0~100) -> [변경] hygiene에서 cleanliness로 수정
    ...
    public String clean() {
        ...
        this.cleanliness = 100; // [변경] hygiene -> cleanliness
    ...
    private void passTime() {
        ...
        // 벌레가 있으면 청결도가 떨어짐
        if (flyCount > 0) {
            this.cleanliness = Math.max(0, this.cleanliness - (flyCount * 2)); // [변경] hygiene -> cleanliness
        }
        
        // 청결도가 낮으면 행복도가 추가로 떨어짐 (악취 패널티)
        if (this.cleanliness < 50) { // [변경] hygiene -> cleanliness
            this.happiness -= 2;
        }
 ...
 // Character.java가 사용할 정적 메소드 (Dimagotchi.isAliveStatic() 호출)
    ...
    public void printStatus() {
        ...
        System.out.println(" 청결도: " + drawBar(100 - cleanliness, true)); // [변경] hygiene -> cleanliness
        System.out.println("-------------------------");
    }
    ...
    //청결도 시간 따라 자동 깎임(밥먹기, 산책, 등등과 시간 로직이 다름)
 // Dimagotchi.java 클래스 내부에 추가
    public void updateStatusByTime() {
        if (!isAliveStatic()) return; // 죽었다면 중지

        // 1. 청결도 감소 (1씩 감소)
        this.cleanliness = Math.max(0, this.cleanliness - 1);

        // 2. 벌레가 있다면 벌레 수에 따라 청결도가 추가로 더 빨리 깎임
        if (flyCount > 0) {
            this.cleanliness = Math.max(0, this.cleanliness - (flyCount * 2));
        }

        // 3. 청결도가 너무 낮으면(악취 패널티) 행복도와 에너지도 조금씩 감소
        if (this.cleanliness < 30) {
            this.happiness = Math.max(0, this.happiness - 1);
            this.energy = Math.max(0, this.energy - 1);
        }

        checkStatus(); // 수치 변화로 인해 죽었는지 확인
    }
 ...
 // Getter 추가
    ...
    // 벌레 및 청결도 관련 메서드
    public int getCleanliness() { return cleanliness; } // [변경] getHygiene -> getCleanliness
    
    public void addFly() {
        this.flyCount++;
        // 벌레가 늘어나면 청결도가 즉시 조금 깎임
        this.cleanliness = Math.max(0, cleanliness - 5); // [변경] hygiene -> cleanliness
    }
    ...
    public String catchFly() {
        if (flyCount > 0) {
            ...
            cleanliness = Math.min(100, cleanliness + 5); // [변경] hygiene -> cleanliness
            addCoins(5); 
            return "벌레를 잡았다! (5G 획득)";
        }
        return "";
    }
 // Dimagotchi.java 파일 안의 메서드
    public void resetAll() {
        // 1. 사망 상태 문자열 비우기 (isAliveStatic이 true가 됨)
